...
def get_import_ids(username, password,wGuid,mGuid):
    user = 'Basic ' + str(base64.b64encode((f'{username}:{password}'
                                            ).encode('utf-8')).decode('utf-8'))
    headers = {'Authorization': user}
    url = f'https://api.anaplan.com/1/3/workspaces/{wGuid}/' + f'models/{mGuid}/imports'
...
def main():
    ...
    if st.button("Send"):
        if username and password:
           Actions = get_import_ids(username, password,wGuid,mGuid)
        else:
           st.write("Failed to retrieve Import ID.")
